...
public class SpellReaderTest {
    ...
    public void testRead() {
        ...
        assertArrayEquals(new Gesture[]{Gesture.UPWARDS, Gesture.DOWNWARDS_LEFT, Gesture.DOWNWARDS_RIGHT}, spell1.rune.gestures);
        assertEquals("rhythmbox-client --play", spell1.magicWords);

        assertEquals(3, spell2.rune.gestures.length);
        assertArrayEquals(new Gesture[]{Gesture.UPWARDS, Gesture.RIGHTWARDS, Gesture.DOWNWARDS}, spell2.rune.gestures);
        assertEquals("rhythmbox-client --pause", spell2.magicWords);

        assertEquals(3, spell3.rune.gestures.length);
        assertArrayEquals(new Gesture[]{Gesture.DOWNWARDS_RIGHT, Gesture.LEFTWARDS, Gesture.DOWNWARDS_RIGHT}, spell3.rune.gestures);
        assertEquals("./lib/lumos.sh", spell3.magicWords);
    }

    @Test
    public void testReadReturnsEmptyListWhenFailsToReadFile() {
        SpellReader spellReader = new SpellReader();
        SpellCaster spellCaster = new SpellCaster();
